...
echo ""
echo "📝 CRITICAL: Add these environment variables in Vercel Dashboard:"
echo "   (Go to: https://vercel.com/dashboard → Your Project → Settings → Environment Variables)"
echo ""
echo "🔑 AUTHENTICATION (REQUIRED):"
echo "   NEXT_PUBLIC_USE_CLERK_AUTH=true"
echo "   NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY=[copy from .env.local]"
echo "   CLERK_SECRET_KEY=[copy from .env.local]"
echo ""
echo "🤖 AI INTEGRATION (REQUIRED):"
echo "   CLAUDE_API_KEY=[copy from .env.local]"
echo ""
echo "💾 DATABASE (REQUIRED):"
echo "   NEXT_PUBLIC_SUPABASE_URL=[copy from .env.local]"
echo "   NEXT_PUBLIC_SUPABASE_ANON_KEY=[copy from .env.local]"
echo "   SUPABASE_SERVICE_KEY=[copy from .env.local]"
echo ""
echo "🌐 APP CONFIGURATION:"
echo "   NEXT_PUBLIC_APP_URL=https://process-audit-116js32e4-myeasysoftware.vercel.app"
echo ""
